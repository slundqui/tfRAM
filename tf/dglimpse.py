...
class GlimpseNet(object):
    ...
    def __call__(self, loc, is_train):
        ...
        g = tf.nn.relu(tf.nn.xw_plus_b(g, self.w_g1, self.b_g1))

        #L pipeline, which encode locations
        l = tf.nn.relu(tf.nn.xw_plus_b(loc, self.w_l0, self.b_l0))
        #l = batchnorm(l, "gn_l0",is_train)
        #l = tf.nn.relu(tf.nn.xw_plus_b(l, self.w_l1, self.b_l1))

        #Combine
        #g = tf.nn.relu(g + l)
        g = g * l
        #Batch norm
        #g = batchnorm(g, "gl", is_train)
        return g
...
class LocNet(object):
    ...
    def __init__(self, config):
        self.loc_dim = config.loc_dim
        self.input_dim = config.cell_size_1
        self.loc_std = config.loc_std
        self.l_size = config.l_size
